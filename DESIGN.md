...
## 2a. Build model — pure Python + a small Rust extension

Since v0.4 the package ships one binary submodule —
`clickhouse_async._fast` — built from a tiny PyO3 crate at the repo
root. The extension carries hot decode primitives that beat
pure-Python loops by avoiding per-row UTF-8 + PyUnicode allocation
and similar byte-level work; everything else (state machine,
codec dispatch, pool, etc.) stays in Python.

### Why Rust at all

The diagnosis behind the v0.4 work (captured in
`.plans/01-rust-overview.md`): clickhouse-connect's Cython-built
hot paths beat our pure-Python equivalents by 4-5× on the 1M-row
read benchmark. The bottleneck is concentrated in a handful of
loops that iterate per row over raw bytes — exactly the workload
where a compiled language wins cleanly. We pick Rust over Cython
because PyO3's ergonomics and toolchain story are friendlier for a
project where most contributors are Python-only.

### Why one combined wheel (not a separate `clickhouse-async-fast`)

Two-package shapes were considered and rejected:

- **Tight coupling** avoids version skew between the Python codec
  layer and the Rust primitives. A shared `__init__.py` keeps the
  public API single-import.
- **One release cycle.** commitizen bumps both pieces together.
- **No optional-extras gating** of performance — the v0.4 stance
  retired the bare-install-stays-import-clean discipline because
  splitting the speed-critical decode path behind an extra would
  bifurcate every test and every benchmark.

The cost is per-platform wheels (linux × {x86_64, aarch64} +
macos × {x86_64, arm64} + win × x86_64 once we publish), instead of
the previous single `py3-none-any` wheel. With `pyo3`'s
`abi3-py311` feature we ship one wheel per OS × arch instead of one
per OS × arch × Python-minor — a meaningful simplification of the
eventual cibuildwheel matrix.

### Why not Cython

Cython requires a C compiler everywhere, leans on `.pyx` syntax
that's neither Python nor C, and produces type-checker output that
ty / mypy don't read natively. PyO3 + Rust gives us:

- Memory safety inside the extension (no UAF, no double-frees).
- A type-checker-friendly `.pyi` stub for the binary submodule
  (hand-maintained, reviewed in PRs that change the Rust signature).
- A toolchain (`cargo` + `maturin`) with first-class IDE / LSP
  support, vs. Cython's bespoke build invocation.
- A mature ecosystem — `orjson`, `pydantic-core`, and `polars` all
  use the same approach.

### When Rust wins (and when it doesn't)

The v0.4 work surfaced a clear discriminator (recorded in
`.plans/04-transpose.md` and `.plans/05-decode-big-int.md`):

- **Rust wins** when the per-cell hot loop touches raw bytes that
  the prior Python path then handed off to multiple primitives —
  varuint parsing, UTF-8 decoding, PyUnicode allocation
  (`String.read`).
- **Rust does not win** when the per-cell hot loop just walks
  already-Pythonised objects through CPython intrinsics
  (`tuple()`, `int.from_bytes`, list-comprehension dispatch).
  CPython is already at the floor; PyO3's `Bound<PyAny>` ergonomics
  add refcount work that exceeds whatever we'd save.

This guides which paths get Rust treatment in subsequent releases:
the candidates are byte-level loops, not object-level loops.
